...
int main() {
    ...
    for(int i = 0; i < t; i++)
    {
        ...
        vector<int64>().swap(factors);
        factors = PollardRho::pollard_rho(n);

        if (adjacent_find(factors.begin(), factors.end()) != factors.end())
            cout << "-1\n";
        else
            cout << factorial(static_cast<int64>(factors.size())) << "\n";
    }
